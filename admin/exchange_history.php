...
// ดึงข้อมูลจาก exchange_requests พร้อมชื่อผู้ใช้และชื่อสิ่งของ
// ปรับปรุง SQL Query ใหม่เพื่อเพิ่มผู้รับคำขอ
// และใช้คอลัมน์ receiver_id จากตาราง exchange_requests
$sql = "SELECT 
    er.id, 
    er.status, 
    er.created_at,
    ...
    i_owner.name AS item_wanted_name,
    i_request.name AS item_offered_name
FROM exchange_requests er
LEFT JOIN users u_requester ON er.requester_id = u_requester.id
LEFT JOIN users u_receiver ON er.receiver_id = u_receiver.id
LEFT JOIN items i_owner ON er.item_owner_id = i_owner.id
LEFT JOIN items i_request ON er.item_request_id = i_request.id
...
<head>
    ...
    <title>ประวัติการแลกเปลี่ยน - ระบบแลกเปลี่ยนสิ่งของ</title>
...
<header>
    <div class="header-left">
        ...
        <nav class="header-nav">
            ...
            <div class="menu-group">
                <a href="exchange_history.php" class="current-page"><i class="fas fa-history"></i> ประวัติแลกเปลี่ยน</a>
...
<div class="container">
    <h2 class="mb-4"><i class="fas fa-history"></i> ประวัติการแลกเปลี่ยน</h2>
        ...
        <table class="table table-striped table-bordered align-middle">
            <thead class="table-dark">
                <tr>
                    <th>#</th>
                    <th>ผู้ร้องขอ</th>
                    <th>สิ่งของที่นำมาแลก</th>
                    <th>สิ่งของที่ต้องการแลก</th>
                    <th>ผู้รับคำขอ</th>
                    <th>สถานะ</th>
                    <th>วันที่</th>
                </tr>
            ...
            <tbody>
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($row['item_offered_name'] ?: '-'); ?></td>
                            <td><?= htmlspecialchars($row['item_wanted_name'] ?: '-'); ?></td>
                            <td><?= htmlspecialchars($row['receiver_name'] ?: '-'); ?></td>
                            <td>
                                <?php
                                switch ($row['status']) {
                                    case 'accepted':
                                        echo '<span class="badge bg-success">ยอมรับแล้ว</span>';
                                        break;
                                    case 'pending':
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6" class="text-center">ไม่มีประวัติการแลกเปลี่ยน</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
