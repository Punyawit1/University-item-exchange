...
$sql = "
SELECT er.id, er.status, er.delivery_method, er.receiver_confirmed, er.requester_confirmed, u.username AS requester_name, er.requester_id,
...
<header>
    <div class="header-left">
        ...
        <nav>
            ...
            <a href="exchange_requests_received.php" class="nav-link-with-badge active">
                    ...
                    <p><strong>สถานะคำขอ:</strong>
                        <span class="status-text
                            <?php
                                ...
                                else if ($row['status'] == 'shipping') echo 'status-info';
                                else if ($row['status'] == 'confirm_receiver' || $row['status'] == 'confirm_requester') echo 'status-info';
                                else echo 'status-completed';
                            ?>">
                            <?php
                                if ($row['status'] == 'shipping') echo 'กำลังจัดส่ง';
                                else if ($row['status'] == 'confirm_receiver') echo 'รอผู้ร้องขอยืนยัน';
                                else if ($row['status'] == 'confirm_requester') echo 'รอผู้รับยืนยัน';
...
<?php endif; ?>
                ...
                <div class="actions">
                    ...
                    <?php if ($row['status'] === 'shipping' || $row['status'] === 'confirm_receiver' || $row['status'] === 'confirm_requester'): ?>
                        <!-- Owner (ผู้รับ) confirm button -->
                        <div style="display:flex; gap:8px; flex-direction:column;">
                        <?php if ($row['receiver_confirmed']): ?>
                            <button class="accept" disabled><i class="fas fa-check"></i> คุณยืนยันแล้ว (ผู้รับ)</button>
                        <?php else: ?>
                            <form method="post" action="process_exchange.php">
                                <input type="hidden" name="request_id" value="<?php echo $row['id']; ?>">
                                <button type="submit" name="action" value="confirm_receiver" class="accept">
                                    <i class="fas fa-shipping-fast"></i> ยืนยันการจัดส่ง (ผู้รับ)
                                </button>
                            </form>
                        <?php endif; ?>
                        ...
                        <?php if ($row['requester_confirmed']): ?>
                            <button class="accept" disabled style="background:#17a2b8;"><i class="fas fa-check-double"></i> ผู้ร้องขอยืนยันแล้ว</button>
                        <?php else: ?>
                            <button class="reject" disabled style="background:#ffc107; color:#333;">รอผู้ร้องขอยืนยัน</button>
                        <?php endif; ?>
                        </div>
                    <?php endif; ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="deliveryForm" method="post" action="process_exchange.php">
                    ...
                    <input type="hidden" name="action" value="choose_delivery">
